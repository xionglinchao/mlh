...
Page({
  ...
  onShareAppMessage: function (res) {
    if (res.from == "button") {
      return {
        title: '美丽花亲子时光',
        path: '/pages/audioVideoCourse/audioCourse/audioCourse?courseId=' + this.data.courseId + '&u_id=' + this.data.mine,
      }
    }
  },
  ...
  getAudioCourseInfo() {
    ...
    let self = this, url = app.baseUrl + '/interface/Course/column_detail', data = {}
    data = {
      'id': this.data.courseId,
      'openid': app.globalData.openId,
      'system': system
  ...
  singleAudioCourseClick(e) {
    ...
    wx.navigateTo({
      url: '/pages/audioVideoCourse/audioPlay/audioPlay?singleCourseId=' + singleCourseId + '&courseId=' + this.data.courseId,
    })
  },
  // 购买课程
  toBuyCourse() {
  ...
  // 问题详情页跳转
  toQuestionDetail(e) {
    let quesId = e.currentTarget.dataset.id
    wx.navigateTo({
      url: '/pages/audioVideoCourse/questionDetail/questionDetail?quesId=' + quesId + '&courseId=' + this.data.courseId,
    })
  }
})